...
class CaseController extends Controller
{
    public function index()
    {
        ...
        return view('main.cases.index', compact(
            'event', 'group', 'isOpened', 'cases',
            'myGuidelines', 'totalGuidelines', 'ownedCount', 'gachaPrice'
        ));
    }
    ...
    {
        $request->validate(['submission_text' => 'required|url']);
        ...
        if($exists) {
            return back()->with('error', 'Sudah disubmit sebelumnya.');
        }

        // TRANSACTION START
        DB::transaction(function () use ($request, $event, $group, $caseId, $user) {

            // Lock dulu untuk hindari race condition
        ...
        return back()->with('success', 'Berhasil submit! Cek saldo & ranking Anda.');
    }
}
