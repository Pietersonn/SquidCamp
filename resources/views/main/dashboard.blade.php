...
@section('styles')
  <style>
    ...
    .dashboard-header {
      ...
      /* Extra padding bawah untuk overlap card */
      color: white;
      position: relative;
    }
    ...
    .balance-card {
      ...
      /* Margin negatif agar naik ke atas header */
...
@section('content')
  ...
  <div class="dashboard-header d-flex justify-content-between align-items-start">
    <div>
      ...
      {{-- INFORMASI (POSISI DITUKAR) --}}
      <div class="d-flex flex-column gap-1">

        {{-- 1. NAMA EVENT (Sekarang di Badge Putih) --}}
        ...
        {{-- 2. NAMA GROUP & ROLE (Sekarang di Badge Transparan) --}}
        @if ($group)
          <div class="event-info-badge text-white">
            ...
            @php
              $myRole = 'Member';
              if ($group->captain_id == Auth::id()) {
                  $myRole = 'Captain';
              } elseif ($group->cocaptain_id == Auth::id()) {
                  $myRole = 'Co-Captain';
              }
            @endphp
  ...
  <div class="balance-card">
    <div>
      <span class="d-block text-muted small fw-bold mb-1"
        style="font-size: 0.7rem; letter-spacing: 1px; text-transform:uppercase;">ASET TIM</span>
      <h2 class="mb-0 balance-amount">
        <span class="currency-symbol">$</span>{{ number_format($group->squid_dollar ?? 0, 0, ',', '.') }}
      </h2>
    </div>
        ...
        <div class="modal-body pt-4">
          <form action="{{ route('main.transaction.transfer') }}" method="POST">
            ...
            <div class="mb-3">
              ...
              <select class="form-select form-select-lg border-0 bg-light" name="to_group_id" required
            ...
            <div class="mb-4">
              ...
              <small class="text-muted mt-1 d-block" style="font-size: 0.7rem;">
                Saldo Tim: ${{ number_format($group->squid_dollar ?? 0, 0, ',', '.') }}
              </small>
            </div>
